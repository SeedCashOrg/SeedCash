...
# from seedcash.hardware.st7789_mpy import ST7789
...
from seedcash.hardware.displays.ili9341 import (
    ILI9341,
    ILI9341_TFTWIDTH,
    ILI9341_TFTHEIGHT,
)
...
class Renderer(ConfigurableSingleton):
    ...
    def initialize_display(self):
        ...
        self.disp = DisplayDriver(
            self.display_type, width=int(width), height=int(height)
        )

        if (
            Settings.get_instance().get_value(
                SettingsConstants.SETTING__DISPLAY_COLOR_INVERTED, default_if_none=True
            )
